...
import {
  deleteRequestAllowedSortColumns,
  deleteRequestColumns,
  deleteRequestGlobalFilterColumns,
} from "./deleteRequest.admin.service";

export const adminDeleteRequestRouter = router({
  list: adminProcedure.input(tableInputSchema).query(async ({ input }) => {
    ...
    const data = await db
      ...
      .from(account_delete_request)
      .where(where)
      .orderBy(orderBy)
      .limit(input.pagination.pageSize)
      .leftJoin(users, eq(account_delete_request.userId, users.id))
      .offset(offset);

    // PostgreSQL returns `bigint` for count → cast to number
    const totalResult = await db
      .select({
        count: sql<number>`count(distinct ${account_delete_request.id})::int`,
      })
      .from(account_delete_request)
      .leftJoin(users, eq(account_delete_request.userId, users.id))
      .where(where);
