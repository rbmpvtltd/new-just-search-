...
async function getPopularCategories() {
  const data = await db
    .select({ photo: category.photo, title: category.title, id: category.id,type:category.type })
    .from(category)
    .where(eq(category.isPopular, true));
  return data;
}
